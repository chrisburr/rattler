...
/// `RATTLER_PKG_CACHE_LAYERS` environment variable.
///
/// The variable accepts a `PATH`-style list of directories (separated by
/// `:` on Unix, `;` on Windows). Each entry is interpreted as a leaf
/// package cache directory in the sense of
/// [`package_cache::PackageCache::new_layered`] — it should contain
/// extracted package subdirectories directly. Layers are searched in the
/// order given, after the primary writable cache from [`default_cache_dir`].
///
/// Returns an empty vector when the variable is unset or expands to no
/// entries.
#[cfg(not(target_arch = "wasm32"))]
pub fn default_pkg_cache_layers() -> Vec<PathBuf> {
    let Some(value) = std::env::var_os("RATTLER_PKG_CACHE_LAYERS") else {
        return Vec::new();
    };
