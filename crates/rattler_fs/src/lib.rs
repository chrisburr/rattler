...
pub enum MountError {
    ...
    #[error(
        "the mount overlay at {} was created for a different version of this \
         environment. It may contain files you want to keep (for example the \
         results of `pip install`). To rebuild the environment from the new lock \
         file, remove the overlay and re-run:\n\n    rm -rf {}",
        .overlay_dir.display(),
        .overlay_dir.display()
    )]
        ...
        join_set.spawn(async move {
            ...
            let cache_metadata = cache
                .get_or_fetch_from_url_with_retry(
                    ...
                    None,
                    // rattler_fs limits concurrency via its own semaphore
...
async fn mount_nfs(
    ...
    // Compiled only on platforms that actually mount NFS: on others the block
    // above diverges, so this tail would be `unreachable_code` (an error under
    // `-D warnings`).
