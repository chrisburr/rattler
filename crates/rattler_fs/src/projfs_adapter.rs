...
unsafe fn get_adapter<T: VfsOps>(data: *const PRJ_CALLBACK_DATA) -> &'static ProjFsAdapter<T> {
    // SAFETY: `data` is a live callback record supplied by ProjFS; its
    // `InstanceContext` is the `Arc::into_raw` adapter pointer registered in
    // `start()`, which the owning `ProjFsHandle` keeps alive for the whole
    // virtualization, so it is valid and outlives every callback.
    unsafe {
...
unsafe extern "system" fn query_file_name_cb<T: VfsOps>(data: *const PRJ_CALLBACK_DATA) -> HRESULT {
    projfs_cb("query_file_name", || unsafe {
        // SAFETY: ProjFS passes a valid `data` pointer for the callback's duration.
...
unsafe extern "system" fn start_dir_enum_cb<T: VfsOps>(
    ...
    projfs_cb("start_dir_enum", || unsafe {
        // SAFETY: ProjFS passes valid `data` and `enumeration_id` for the call.
        let adapter = get_adapter::<T>(data);
        let path = pcwstr_to_path((*data).FilePathName);
...
unsafe extern "system" fn get_placeholder_info_cb<T: VfsOps>(
    ...
    projfs_cb("get_placeholder_info", || unsafe {
        // SAFETY: ProjFS passes a valid `data` pointer for the callback's duration.
        let adapter = get_adapter::<T>(data);
        let path = pcwstr_to_path((*data).FilePathName);
...
unsafe extern "system" fn get_file_data_cb<T: VfsOps>(
    ...
    projfs_cb("get_file_data", || unsafe {
        // SAFETY: ProjFS passes a valid `data` pointer for the callback's duration.
        let adapter = get_adapter::<T>(data);
        let path = pcwstr_to_path((*data).FilePathName);
